...
Let's say at Jigisa, for every student we assign a Buddy. For this we have a `students` table, which looks as follows:
...
You are asked to print the names of everyone associated with Jigisa. So, in the result we will have one column with all the names.
